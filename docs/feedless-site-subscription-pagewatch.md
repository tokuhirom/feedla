...
| 6.5 | `tools/htmlskeleton` + 匿名化検査テスト（§14.5） | 1 | MVP 2 サイトのフィクスチャが生成でき、検査テストが緑 |
...
- **著作権・利用規約**: サイト運営者は全文配信を意図していない。
  実行時の扱いは登録時の注意喚起（§9.1）に留め、それ以上の制限は設けない
  （取得したページを手元の DB に保存するのは私的利用の範囲）。
  一方**開発面では、実在サイトの HTML をリポジトリに置かない**という
  はっきりした線を引く。公開リポジトリへのコミットは再配布であり、
  私的利用では説明がつかない。テストには §14.5 のツールで構造だけを
  抽出した匿名化フィクスチャを使い、匿名化されていることを CI で検査する。
...
「1 週間動かして観察」では取りこぼしを検証できない。**構造を模した
合成フィクスチャによるゴールデンテスト**を主、実サイトでの手動確認を従とする。

> **実在サイトの HTML をリポジトリに置いてはいけない。**
> feedla は公開リポジトリであり、第三者が書いた日記本文をコミットすることは
> 再配布にあたる。取得したページをローカルの DB に保存するのは私的利用の
> 範囲だが、**リポジトリに入れた瞬間に別の話になる**。§14.1/§14.2 に
> 記録してよいのは「どういう構造か」という事実（タグ名・クラス名・
> ヘッダの有無）までで、本文そのものは含めない。

1. **ゴールデンテスト**（必須・CI で常時実行）:
   実ページから**構造だけを機械的に抽出した匿名化フィクスチャ**を
   `internal/extract/pagewatch/testdata/` に置く（§14.5 のツールで生成）。
   手で似た HTML を書き起こすのではなく実物から生成するのは、
   **手書きだと「自分が気づいた構造」しか再現できない**ため。
   ノイズ源は往々にして見落とした側にある。

   「旧版 → 新記事 1 件追加 → 同一 → `lastmod` だけ変更 →
   古い記事が押し出された版」の順に食わせ、
   エントリー数が `1, 1, 0, 0, 0` になること。
2. **実サイトでの手動確認**（従・CI では実行しない）:
   環境変数 `FEEDLA_PAGEWATCH_FIXTURE_DIR` に手元のディレクトリを指した
   ときだけ走るテストを用意する。未設定なら `t.Skip`。
   手元に保存した実ページを食わせて、匿名化で失われた要素
   （実際の文言に依存する `ignore_patterns` の効き方など）を確認する。
   **このディレクトリはリポジトリの外**に置く。
3. **実運用観察**（従）: 2 サイトを 2 週間監視し、誤検知が 0 件であること。
   更新が来れば取りこぼしも確認するが、来ない可能性が高い。

### 14.5 構造抽出ツール `tools/htmlskeleton`

**実ページ → 匿名化フィクスチャ**を生成する開発用ツールを用意する
（`go run ./tools/htmlskeleton -in saved.html -out testdata/diary-v1.html`）。
feedla 本体のバイナリには含めない独立の `main` パッケージ。

**変換規則**

| 対象 | 扱い | 理由 |
|---|---|---|
| テキストノード | **文字種を保って 1 文字ずつ置換**（ひらがな→`あ`、カタカナ→`ア`、漢字→`亜`、ラテン小文字→`a`、大文字→`A`、数字→`0`、記号・空白はそのまま） | 文字数・文字種・単語境界・空白の入り方が保たれ、ブロック分割や長さ上限の挙動が変わらない。意味は完全に失われる |
| `class` / `id` / `name` / `role` | **そのまま残す** | §4.2 の除去規則が効くかどうかがテストの主題。ここを潰すと何も検証できない |
| `href` / `src` | パス構造は残す | 相対 URL の絶対化・トラッキングパラメータ除去の検証に要る |
| その他の属性 | そのまま残す | §4.3 の属性フィルタの検証に要る |
| コメントノード | 中身を空にする | 構造（コメントがある事実）だけ残す |
| `<title>` | テキストと同じ規則で置換 | |

`-keep <regexp>`（複数指定可）で、**マッチしたテキストノードを置換しない**。
`Last Modified:` や `Document ID:` のような定型句は、創作的表現ではなく
かつ `ignore_patterns` の検証に必要なので残す。何を残したかは生成物の
先頭コメントに記録する。

**生成物の先頭に必ず入れるヘッダ**

```html
<!-- generated by tools/htmlskeleton from https://example.net/memo/ on 2026-08-16
     structure only: all text has been replaced. -keep: ^Last Modified: -->
```

**匿名化されていることを CI で機械検査する**

`testdata/` 配下の全フィクスチャについて、
「テキストノードが匿名化後の文字集合（`あアaA0` + 記号・空白）と
`-keep` パターンにマッチする部分だけで構成されている」ことを検証する
テストを置く。**うっかり生ページをコミットする事故を機械的に防ぐ**のが目的で、
これがあると「著作権に配慮する」が人間の注意力ではなく仕組みになる。

**副次的な効果**: 「あのサイトで誤検知した」という報告に対し、
そのページから匿名化フィクスチャを 1 コマンドで作って回帰テストに
できる。サイトが増えるほど除去規則の変更が安全になる。
