   ...
   └─ ClaimDueFeeds ──▶ crawlOne(feed)
        ...
        internal/extract/selector.Extract(html, prevState)
            ...
            既存 crawler.normalizeItem → store.UpsertEntries（無変更）
                          │
                          ▼
            scrape_sources.state を更新（取り込み結果に応じて seen / pending を更新。§6.5）
```
...
**既にビルドに入っている**。`go.mod` の `// indirect` を外して直接依存に昇格させるだけで、
**モジュールは 1 つも増えない**（これまで使っていなかった API を呼べば
リンカのデッドコード除去の関係で数 KB は増えうるが、実質的には変わらない）。
...
|---|---|---|
| `item_selector` | **必須** | — |
| `link_selector` | 任意 | item 自身が `<a href>` ならそれ。無ければ item の子孫の `<a href>` のうち**リンクテキストが最も長いもの**（同点なら文書順で先） |
| `title_selector` | 任意 | §4.7 のフォールバック連鎖に従う |
...
- `item_selector` は**ページ全体に対する**セレクタ、残りは**各 item 要素に対する
  ...
  ただし意味論は DOM の `element.querySelectorAll` と同じで、
  **item の外側の祖先もマッチ判定に参加する**（結果集合が item 配下に
  限定されるだけ）。`:scope` 相当は無いので、`.list > .title` のような
  祖先付きセレクタは「item の外の `.list`」にも反応しうる。
- `item_selector` が入れ子でマッチした場合（`article` の中の `article` など）は、
  **外側にマッチした item の子孫を item として扱わない**（外側だけを採る）。
  URL の重複排除でほぼ吸収されるが、明記しておく。
...
フォールバック連鎖は**ここに一本化する**（§4.2 の表はここを参照するだけ）。
`fulltext` の値で終端が変わるので、モード別に書き切る。

**`fulltext = true`（既定。記事ページを取得する）**

1. `title_selector` のテキスト（指定されていれば）
2. リンク要素のテキスト
3. item 全体のテキストを 120 文字に切ったもの
4. 記事ページの Readability タイトル
5. 記事ページの `<title>`
6. 記事 URL のパス末尾

**`fulltext = false`（記事ページを取得しない）**
...
（4〜5 は記事ページが手元に無いので実行できない。この連鎖を
書いておかないと、`fulltext = false` でタイトルが空のときの挙動が未定義になる。）
...
`CommitState` は**呼ばれるたびに、今回の候補集合に含まれない `pending` キーを削除する**。
（候補が 0 件のクロールでは `CommitState` 自体が呼ばれない（§6.5）ので、
掃除は次に候補が出たクロールまで遅れる。実害はない。）
...
type CommitInput struct {
    // Candidates は今回の新着候補すべて（文書順）。着手しなかった分も含む。
    Candidates []string
    // Imported はエントリーを作った URL。本文が取れたか、タイトルだけかは問わない
    // （§4.5 の表で「seen に入れる」と書いた行がすべてここに入る:
    //  本文抽出失敗・robots Disallow・3 回目の諦め）。
    Imported []string
    // FetchFailed は記事ページの取得に失敗した URL（§4.5 の 1 行目）。
    // pending を +1 し、3 回目に達したものは Imported 側で処理される。
    FetchFailed []string
    // Initial は state が NULL だった初回クロールか。
    Initial   bool
    Truncated bool
}
```
...
1. `Imported` を `seen` に入れる。
2. `FetchFailed` の `pending` を +1 する。
3. **`Initial` が真なら、`Candidates` のうち `FetchFailed` 以外を全部 `seen` に入れる**
   （§4.5 の「初回は残りを封印する」）。偽なら未着手の候補は `seen` に入れない
   （§4.5 の「後回し」）。**この 1 ビットが無いと両者を区別できない。**
4. `Candidates` に含まれない `pending` のキーを削除する（§6.1.2）。
5. `seen` が上限を超えていたら、`Candidates` に**含まれない**もののうち
   追加順の古いものから捨てる（§6.1.1）。

`Candidates` を渡すのは 3・4・5 のすべてに必要だから。
...
**着手しなかった候補は `UpsertEntries` に渡さない。** `Extract` が返す
`Feed.Items` は新着候補**全部**（最大 500 件）だが、crawler は
`max_items_per_crawl` 件に**トリムしてから**書き込みパイプラインに流す。
これを書いておかないと、§2 の図からは「未着手分も本文なしエントリーとして
先に INSERT する」実装も読めてしまう。そうすると本文が空のエントリーが
大量に未読で並び、次のクロールで本文が埋まる（UPDATE）という
分かりにくい挙動になる。
...
- レスポンスは kind ごとに形が違う。pagewatch は既存の `{blocks: [...]}`、
  ...
  ```json
  {
    ...
    "truncated": false,
    "warnings": [
      "3 件の要素にリンクが無く、スキップされました",
      "5 件の要素が同じ URL を指していたため 1 件に畳まれました。link_selector の指定を検討してください"
    ]
  }
  ```
...
1. **`GET /api/v1/scrape_sources` と `GET /api/v1/scrape_sources/{id}` が、
   全ユーザーのソースを無条件に返す**（一覧は `store.ListScrapeSources` に
   ユーザー絞り込みがなく、by-id の `handleGetScrapeSource` にも所有チェックがない。
   **一覧だけ絞っても by-id が開いていれば意味がない**ので、対処するなら両方）。
...
**同じ Config を GUI で埋める**形になる（§10.7）。
...
以下 §10.2〜10.5 はその安全な表示の設計、§10.6 がセレクタ生成である。

### 10.2 前提: srcdoc の iframe は親の CSP を継承する

方式を比べる前に、ブラウザ側の確定事項を 1 つ押さえる。

> `srcdoc`（および `about:blank`、`blob:`、`data:` などのローカルスキーム）で
> 作られた文書は、**埋め込み元の文書の policy container を継承する**。
> つまり **feedla 本体の CSP がそのまま iframe の中身にも適用される**。

これは実装依存の挙動ではなく仕様で決まっている。現行の CSP は

```
default-src 'self'; img-src 'self' https: data:; script-src 'self'; frame-src https://www.instagram.com
```

なので、素朴に `<iframe sandbox="allow-scripts" srcdoc="...">` へ流し込むと:

- **クリック検出用に注入するインラインスクリプトが `script-src 'self'` でブロックされる**
  （nonce も hash も無いため）。GUI の中核が動かない。
- **元ページのインライン `style` 属性と `<style>` もブロックされる**
  （`style-src` が無いので `default-src 'self'` にフォールバックし、
  インラインスタイルは `'unsafe-inline'` なしでは効かない）。
  スタイルが一切当たらないので、**見た目は「タグとテキストだけ」になる** —
  すなわち §10.1 の表で ✕ を付けた方式 C に退化する。

また逆に、「`frame-src` に `'self'` を足す」という対処は**不要かつ無効**である。
`srcdoc` はネットワークナビゲーションを伴わないので `frame-src` の判定対象にならず
（だからこそ代わりに親の CSP が継承される）、仮に対象だったとしても
`'self'`（scheme + host + port の照合）が `about:srcdoc` にマッチすることはない。

**この事実が、次節の方式選定をそのまま決める。**

### 10.3 表示方式の比較

| 方式 | 見た目の再現 | 外部への通信 | アプリ本体の CSP | 判定 |
|---|---|---|---|---|
| A. `srcdoc` + 親 CSP を緩める | 中 | なし | **恒久的に緩む** | ✕ |
| **B. 専用エンドポイントを実ナビゲーションで読ませ、そのレスポンスに個別の CSP を付ける（採用）** | 中 | なし | **触らない**（`frame-src 'self'` の追加のみ） | ◎ |
| C. 構造だけのアウトライン表示 | なし | なし | 触らない | △ |
| D. 外部 CSS/画像を feedla 経由でプロキシ | 高 | feedla のサーバから発生 | — | ✕ |
| E. サーバ側でスクリーンショット（headless） | 完全 | あり | — | ✕（DESIGN.md の中核目標と衝突） |

**A を採らない理由**が §10.2 から出てくる。A を成立させるには親の CSP に

- picker スクリプトの `'sha256-…'`（固定内容なのでハッシュ指定は可能）
- **`style-src 'unsafe-inline'`**

を足すことになるが、後者は**アプリ全体に効く**。feedla の本文ペインは
第三者が書いた HTML をサニタイズして表示する場所であり、そこの
インラインスタイル禁止を、プレビュー機能のために恒久的に解除するのは
明確に割に合わない。

**B を採用する。**

- `GET /api/v1/scrape_sources/inspect/view?t=<token>` が、サニタイズ済みの
  HTML（+ picker スクリプト）を**通常のレスポンスとして返す**。
  iframe はこれを `src` で読む（実ナビゲーション）。
- ナビゲーションで読まれた文書は**親の CSP を継承せず、自分のレスポンスヘッダの
  CSP に従う**。したがってこのレスポンスにだけ次を付ける:

  ```
  Content-Security-Policy: default-src 'none'; style-src 'unsafe-inline'; script-src 'sha256-<picker>'; img-src data:
  ```

  `default-src 'none'` なので、**サニタイズを擦り抜けた外部参照があっても
  ネットワークに出られない**（サニタイズと CSP の二重防御になる）。
  `script-src` をハッシュ 1 個に固定しているので、picker 以外のスクリプトは
  たとえ残っていても動かない。
- iframe には引き続き `sandbox="allow-scripts"` を付け、
  **`allow-same-origin` は付けない**。iframe は不透明オリジンになり、
  親の DOM・Cookie・`localStorage`・feedla の API セッションには到達できない。
- 親の CSP には `frame-src 'self'` を足す。**B では実ナビゲーションなので
  今度は本当に必要になる**（現在 `frame-src` は embed 用に存在するので値を 1 つ増やすだけ）。

**認証をどうするか（B 固有の注意点）**: `sandbox` の不透明オリジンから発する
リクエストはサイト間扱いになりうるため、**SameSite Cookie が送られない可能性がある**。
セッション Cookie に依存しない形にする:

- `POST /api/v1/scrape_sources/inspect`（§8.3）が、認証済みリクエストとして
  ページを取得・サニタイズし、結果をサーバ側に短命（数分）で保持したうえで、
  **1 回限りの不透明トークン**を含む view URL を返す。
- `GET …/inspect/view?t=…` はそのトークンだけで認可する。トークンは
  発行したユーザーに紐づけ、使い捨て・短命とする。
...
feedla は本文のサニタイズに bluemonday の**許可リスト**を使っている
（`crawler.bodyPolicy`）。**ページ丸ごとを表示する F2 では攻撃面がさらに広いので、
除去リスト（ブラックリスト）で書いてはいけない。**

当初は「`<script>`・`on*`・`javascript:`・`<object>`/`<embed>`・
外部参照要素を除去する」という列挙で書いていたが、それだけでは
少なくとも次が漏れる:

| 漏れていたもの | 何が起きるか |
|---|---|
| `<meta http-equiv="refresh">` | **スクリプト無しで**第三者 URL へ自動ナビゲート。「プレビューを開いただけで相手サイトに IP と Referer が渡らない」という方式選定の根拠が、これ 1 つで破れる |
| `<a href>` のクリック | sandbox は自フレームのナビゲーションを禁じない。picker が動かなかった場合、リンクを踏むと iframe が第三者サイトへ遷移する。`preventDefault` はスクリプト依存の防御なので、サニタイズ側で無効化すべき |
| インライン `style` 属性の `url(...)` | `<style>` の `url()` は除去すると書いたが、属性側に触れていなかった |
| `srcset` / `imagesrcset` / SVG の `<image href>` / `<use href>` | 外部取得 |
| `<base>` | 相対 URL の解決先をすり替える |
| `<form action>` | sandbox 既定で送信は塞がるが、除去が筋 |

したがって規則を次のとおりにする。

- **残すタグ**: 構造とテキストに関わるものだけ（`div`,`span`,`p`,`h1`〜`h6`,
  `ul`,`ol`,`li`,`dl`,`dt`,`dd`,`table`,`thead`,`tbody`,`tr`,`th`,`td`,
  `article`,`section`,`header`,`footer`,`nav`,`aside`,`main`,`figure`,
  `figcaption`,`time`,`a`,`img`,`strong`,`em`,`b`,`i`,`small`,`br`,`hr`,`style`）。
  **それ以外は要素ごと落とす。**
- **残す属性**: `class`,`id`,`style`（`url()` を除去したもの）,`data-feedla-id`,
  `alt`。**それ以外は全部落とす**（`href` も `src` も落とす。
  リンク先は選択には不要で、残すとナビゲーションの起点になるだけ）。
- `img` は `alt` テキストのプレースホルダ要素に置換する（レイアウトの
  当たりを付けるため、元のサイズが分かれば `style` で箱だけ再現してもよい）。
- **picker スクリプトは文字列連結で末尾に足さない。** `x/net/html` の
  ツリーに要素として挿入してから再シリアライズする
  （未閉タグや未終端コメントの中にスクリプトが飲み込まれる事故を防ぐ）。

`class` / `id` を残すのは、それがセレクタ生成の材料そのものだから
（F0 の `tools/htmlskeleton` が同じ理由で class/id を残す判断をしている）。

### 10.5 クリックからセレクタを作る

`allow-same-origin` を付けない iframe は不透明オリジンになるため、
**親ウィンドウから `contentDocument` には触れない**。したがってクリック検出は
iframe の中で動く JS が担う（これが `sandbox="allow-scripts"` を付ける理由）。

構成:

1. `POST /api/v1/scrape_sources/inspect`（§8.3）が、サニタイズ時に
   **各要素へ `data-feedla-id="N"` を振り**、あわせて要素インデックス
   （id → タグ名・class 集合・親子関係）を JSON で親に返す。
2. iframe 内の picker スクリプトは、click を拾って
   **`data-feedla-id` の整数 1 個だけ**を `parent.postMessage` で送る。
3. 親ウィンドウは受け取った id を、**サーバ由来のインデックス**（1 で受けたもの）に
   引き当ててセレクタを組み立てる。iframe が送れるのは整数だけなので、
   **iframe 側から任意の文字列が親のロジックに入ることがない**。

picker スクリプトはホバー中の要素にも `outline` を当てる（クリック対象の予告表示）。
これは iframe 自身の DOM を触るだけで `postMessage` を経由しないため、
`allow-same-origin` なしでも実装でき、上記の安全性の根拠に影響しない。

安全性の根拠:

- サニタイズが許可リスト（§10.4）— 主防御
- レスポンス個別の CSP `default-src 'none'`（§10.3）— 擦り抜けてもネットワークに出られない
- `allow-same-origin` なし — 親にも feedla のセッションにも到達できない
- 親が受け取るのは整数 1 個だけ — インターフェースの最小化

**`event.origin` で検証してはいけない**。不透明オリジンからの `postMessage` は
`event.origin` が**文字列 `"null"`** になり、これはページ上のあらゆる
不透明オリジン（別の sandbox iframe など）と区別が付かない。
検証は **`event.source === iframeEl.contentWindow`** の同一性チェックと、
ペイロードが整数であり inspect のインデックスに存在する id であることの
確認で行う。（origin が `"null"` になる事実をここに書き残しておくのは、
後から「origin チェックを足しておこう」と再発させないため。）

子から親への `postMessage` の `targetOrigin` は親のオリジンを明示指定できる。
逆に将来「親 → 子」の通信を足す場合は、宛先が不透明オリジンなので
`"*"` にせざるを得ない — この非対称性に注意する。

**実装時に検証すべき点**（確定事項と要検証を分けて書く）:

- （確定・仕様）`srcdoc` は親 CSP を継承する。だから B を採る（§10.2）
- （確定）不透明オリジンからの `postMessage` は届く。`event.origin` は `"null"`
- （**要検証**）B の view エンドポイントを sandbox iframe が読むとき、
  セッション Cookie が送られるか。送られない前提でトークン認証を設計してあるが、
  実際の挙動を確かめてトークンの要否を確定する
- （**要検証**）レスポンス個別 CSP の `script-src 'sha256-…'` が
  意図どおり picker だけを通すこと（ハッシュ計算対象の空白・改行の扱い）

F2 はこの 4 点を小さなプロトタイプで確かめてから本実装に入る。

### 10.6 セレクタ生成のアルゴリズム

**注意すべき点**: `item_selector` に必要なのは
**「全 item にマッチするセレクタ」**であって、クリックされた 1 個を
一意に指すセレクタではない。「クリックした要素を一意に指す」方向で
`nth-of-type` にフォールバックすると、**1 件しかマッチしないセレクタ**が
生成され、記事 1 本しか拾わない購読ができあがる。

したがって生成物は次の 2 部構成とする。

```
<リスト要素（親）を一意に指すセレクタ>  >  <item の共通部分>
例:  #news-list > li
     .archive .entries > article.post
```

1. **記事コンテナ候補を決める**: クリックされた要素から祖先を辿り、
   **同じ「構造シグネチャ」を持つ兄弟が 3 個以上ある**最小の祖先を採る。
   構造シグネチャ = タグ名 + class 集合（不安定 class を除く）。
   - **兄弟が 3 個未満のとき**のフォールバック: 文書全体で同一シグネチャを持つ
     要素が N 個以上あるかを見る。グリッドレイアウトで
     `<div class="row">` に 3 件ずつ包まれている場合など、
     **item が同一親の兄弟にならないレイアウトは実在する**ので、
     「兄弟」だけを条件にすると失敗する。
   - 記事が 2 本しかない一覧でも失敗するので、しきい値 3 は
     「候補のスコアリング」に使い、下回っても候補として提示はする。
2. **親（リスト要素）を一意に指すセレクタを作る**: `#id` → `tag.class`
   （文書内で一意なら）→ さらに上の祖先を足す → 最後の手段として `nth-of-type`。
   **`nth-of-type` を使ってよいのはこちら側だけ。**
3. **item の共通部分を作る**: タグ名 +（全 item に共通する）安定 class。
4. **不安定な class を除外する**: 長い 16 進文字列、ランダムに見える英数字列
   （CSS Modules / CSS-in-JS のハッシュ）、数字のみ。
   これを入れないと、ビルドのたびに壊れるセレクタを生成してしまう。
5. **候補を 2〜3 個提示し、それぞれのマッチ件数を添えて選ばせる。**
   **マッチが 1 件のセレクタは候補から除外する**（健全性チェック）。

`title_selector` 等は、記事コンテナを決めたあとに
「その中でタイトルらしき要素をクリック」→ コンテナからの相対セレクタを生成、
という同じ流れで作る。こちらは item 内で一意に決まればよいので、
`nth-of-type` へのフォールバックが許される。

### 10.7 F1 の設計が F2 を縛っていないことの確認
...
| 抽出・取得・エントリー化 | ◎ 一切変更なし |
| 追加で要るもの | `inspect` + `inspect/view` エンドポイント（§8.3 でパスを予約）、親 CSP への `frame-src 'self'` 追加、許可リスト方式のサニタイザ、iframe とセレクタ生成の UI |

**F2 は「Config を埋める別の入力手段」に閉じており、F1 の後方に何も要求しない。**
F1 側で先に決めておくべきことは §8.3 のパス予約だけで、
CSP・サニタイザ・GUI はいずれも F2 の中で閉じる。
...
| state 単体 | `CommitState` の 5 規則（§6.5）を 1 つずつ:<br>・`Imported` が `seen` に入り、`FetchFailed` の `pending` が +1 されること。3 回目で `seen` に入り `pending` から消えること<br>・**`Initial=true` のとき未着手の候補も `seen` に封印されること／`false` のとき封印されないこと**<br>・候補集合に無い `pending` キーが削除されること<br>・**`seen` 上限超過の追い出し対象が「候補集合に無い URL」に限られること**（候補に載っている古い URL は残る）<br>・**セレクタ変更（config_hash 不一致）で `seen` が捨てられないこと**<br>・**壊れた JSON / 未知 `version` で再同期モードになること**（Items が空、候補が全部 `seen` に入る、記事 fetch が 0 件） |
...
- **エントリー単位の「これは記事ではない」除外導線は設けない**（F0 §9.4 の
  「このブロックを無視する」に相当するもの）。誤爆の回復はセレクタの修正で行う。
  F0 では誤検知が毎回通知として飛んでくるので回復導線が必須だったが、
  本方式は `item_selector` でスコープが絞れているぶん誤爆の質が違い、
  単発の混入はそのエントリーを既読にすれば済む。
  実運用で単発除外の需要が出たら `exclude_urls []string` を Config に足す
  （**この判断は明示しておく。書かないと「検討漏れ」と区別が付かない**）。
- **一覧の 1 ページ目しか見ない**（§4.4）。購読開始より前の記事は、
  初回に取り込む先頭 `max_items_per_crawl` 件を除いて取り込まれない
  （残りは初回に `seen` へ封印される。§4.5）。
...
- **恒久リダイレクト時に `scrape_sources.target_url` がずれる**（F0 からの既存問題）。
  ...
  **`feed_url` を更新するときは `target_url` も併せて更新する**ことを
  実装に含める（作業分解 #7.5）。
- ~~購読者がゼロになった feed もクロールされ続ける~~ → **マルチユーザー化 Phase C
  (2026-08-17、`docs/multi-user-design.md`) で解消済み。** `ClaimDueFeeds`/
  `ListDueFeeds` が購読者ゼロの feed を kind 問わず除外し、`Store.DeleteOrphanFeeds`
  が「購読者ゼロ かつ 最終活動から7日経過」の feed を削除する(`internal/store/gc.go`)。
  selector 固有の対処はしていないが、この節を書いた時点で想定していた「孤児ソースが
  1+N リクエストを叩き続ける」問題自体が feed の kind に関わらず一般解決されたため、
  本方式にも同じく適用される。
