...
- 日次で `VACUUM INTO '<FR_BACKUP_DIR>/feedla-YYYYMMDD.db'`（WAL 中でも安全。
  ...
  ようにするため)。ただし初期セットアップ未完了(bootstrap admin が
  sentinel パスワードのまま)の間はバックアップ自体をスキップする。
  復旧に失敗して空 DB で起動してしまった個体が、空のスナップショットを
  当日キーで上書きアップロードし、世代 prune で本物のバックアップを
  押し出していく事故を防ぐため。
...
- `feedla serve` 起動時、`FR_DB_PATH` の DB ファイルが存在しなければ
  `internal/restore/` が自動復旧を試みる。ローカル(`FR_BACKUP_DIR`)・
  リモート(`FR_BACKUP_REMOTE_*`)の両方からファイル名(YYYYMMDD)で
  最新のスナップショットを選ぶ(古いローカルが新しいリモートを隠さない)。
  リモートの列挙に失敗してもローカル候補があればそちらで続行する。
  どちらにもなければ従来どおり空の DB で起動する(エラーにはしない)。
  既存 DB がある場合は一切手を出さない。
- 自動復旧が走らなかった/何も見つけられなかった場合でも、初期セットアップ
  画面から手動で復元できる。`GET /api/v1/auth/me` の `restore_hint` が
  復元候補(最新スナップショットのファイル名と local/remote の別)を返し、
  Web UI は「バックアップから復元する」か「新しく管理者アカウントを作成」
  かの二択を出す。復元を選ぶと `POST /api/v1/auth/restore`(セットアップ
  未完了時のみ有効、公開エンドポイント。セットアップ完了後は 409)が
  スナップショットを `FR_DB_PATH+".restore"` にステージし、サーバは
  goroutine 群を graceful に畳んでからファイルを差し替え、同一プロセス内で
  サーバスタックを再起動する(`cmd/feedla` の restore-and-restart ループ。
  外部のプロセススーパーバイザに依存しない)。SPA は 202 を受けたら
  `/auth/me` をポーリングして復元後の画面(通常はログイン)へ遷移する。
...
1. **フィード内の全文取得（本文抜粋しか流さないフィードへの対応）** —
   ...
   `feed_boilerplate` テーブルに JSON で持つ）。両経路（`feed_fulltext` と
   selector 方式の記事取得）が同じ状態を共有する。

   除去の条件は「2ページ以上で観測」「観測ページ数の半分以上に出現」「テキストの
   半分以上がリンク」の3つすべて。**繰り返しだけを根拠に消してはいけない**のは、
   全記事に同一 HTML で載る本文ブロック（連載の共通イントロ、定型の注意書き）が
   繰り返しの観点ではサイト共通部分と区別できないため。この誤除去は「本文が少し
   欠けただけの、長さも形も正常な本文」を生むので、抽出結果を見ても検出できず、
   そのまま `entries.body` に恒久保存されてしまう。リンク密度の条件は、この
   区別を「ナビ・パンくず・フッタ・関連記事リストはリンクが主体、繰り返す散文は
   そうでない」という形で近似するもの。取りこぼし（リンクを含まない共通の定型文が
   本文に残る）は許容する — 読者から見えるノイズは後から直せるが、失われた本文は
   直せない。

   除去後の抽出が失敗する/短すぎる/元の抽出結果の 1/4 未満しか残らない場合は
   除去なしの結果を使う。これらが守るのは「ほぼ同一のページが並ぶフィードを学習
   させてしまい、本文ごと消える」ような壊れ方であって、上記の部分欠落ではない。
   `<head>`（特に `<base href>`）は除去対象外 — これを落とすと相対 URL の解決基準が
   変わるが、テキスト長は変わらないため上記のどのチェックにも引っかからない。

   学習状態が肥大化しないよう、候補にするのは文書ルートから 10 段以内・テキスト
   32 文字以上のサブツリーに限る（サイト共通部分は文書の上部にある）。観測ページ数が
   64 に達したら全カウンタを半減させ、サイトリニューアル後も新しい共通部分に
   追随できるようにしているが、更新頻度の低いフィードでは収束に数十ページかかる。
