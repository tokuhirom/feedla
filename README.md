...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `FR_LISTEN` | `127.0.0.1:8080`（Docker イメージでは `0.0.0.0:8080`） | コンテナ**内**の待受アドレス。Docker の `-p` によるポートマッピングを機能させるため、コンテナ内は `0.0.0.0` のまま変更しない。ホスト側の公開範囲は `docker run -p <ホストIP>:8080:8080` の `<ホストIP>` 側で制御する(クイックスタートは `127.0.0.1` 限定がデフォルト)。 |
