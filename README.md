...
自分専用のセルフホスト型 RSS/Atom リーダーです。デスクトップでは livedoor Reader /
Fastladder を彷彿とさせる LDR 風 3 ペイン UI でキーボードだけで大量の記事を高速に
読み流せ、スマホではスワイプで記事を読み進められるネイティブアプリ的な UI に
自動で切り替わります。単一バイナリ + SQLite だけで動き、外部 DB やジョブキューは
不要です。
...
- **デスクトップ: LDR 風の 3 ペイン UI**。キーボードだけで大量の記事を読み流せる。
- **スマホ: ネイティブアプリ的な UI**。狭幅では 1 画面ずつの表示に切り替わり、
  スクロールで既読になる。左右スワイプで次 / 前の記事へ移動でき、長い記事を
  最後までスクロールせずに読み飛ばせる。
...
アプリ内で `?` を押すと一覧が表示されます。着地条件や既読になる範囲まで含めた
詳細な仕様は [docs/keyboard-shortcuts.md](docs/keyboard-shortcuts.md) を参照して
ください。
...
日次バックアップのタイマーは固定時刻(深夜など)ではなく、**`feedla serve`
プロセスの起動時刻から 24 時間ごと**に発火します。そのため起動時、当日分の
バックアップ(`feedla-<今日の日付>.db`)がまだ存在しない場合は、その場で
即座に 1 回分のバックアップを取ってから通常の 24 時間サイクルに入ります
(再起動を挟んでもその日のバックアップが取れないまま長時間放置されるのを防ぐため)。
...
管理者アカウントでログインすると、Web UI の「管理者用ツール」画面の
「バックアップ」タブに、ローカル(`FR_BACKUP_DIR`)・リモート
(`FR_BACKUP_REMOTE_*`)それぞれに実際に存在するバックアップファイルの一覧
(ファイル名・サイズ・更新日時)を確認できます。ホスト側にログインしなくても
バックアップが本当に取れているか確認できるようにするための機能です。
