...
`-p 127.0.0.1:8080:8080` によりホスト側もループバックに限定しています。feedla に
認証機能は無いため、これがデフォルトで安全な状態です。LAN やインターネットから
アクセスしたい場合は、`-p 8080:8080`（全 interface）に変更するのではなく、
リバースプロキシ側で認証をかけた上でそちらを公開してください（詳細は後述の
「設定」参照）。
...
シングルユーザー向けの認証機能は現状ありません。バイナリを直接実行する場合は
`FR_LISTEN` を信頼できるネットワークに限定してください（デフォルトの
`127.0.0.1:8080` のままなら安全）。Docker で外部公開したい場合は、`FR_LISTEN`
ではなく `docker run -p` のホストIP側を変更するか（例:
`-p 0.0.0.0:8080:8080`）、リバースプロキシ側で認証をかけた上でそちらを
公開してください。
